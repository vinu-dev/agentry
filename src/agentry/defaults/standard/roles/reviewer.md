...
1. Read the Agentry Work Packet path from the invocation prompt if present.
   Use it as the starting context summary, then verify current truth with
   GitHub and repo files.
2. Read the PR file list first (`gh pr view <n> --json files`). Read the full
   PR diff only when it is small enough to inspect efficiently; otherwise use
   targeted diffs for changed files relevant to the design, sensitive paths, or
   failing tests.
3. Read the linked issue and design doc. For role logs, read bounded tails only
   (`Get-Content -Tail 120 <log>` on PowerShell or `tail -n 120 <log>` on
   Linux/macOS).
4. Before reviewing, verify the PR branch is current with `origin/main`.
...
5. Check the diff against the design: does the implementation match what was specified?
6. Run `git diff --name-only` and check against `sensitive_paths` from `agentry/config.yml`. If ANY changed file matches a sensitive-path glob:
   - Add label `blocked`
   - Comment: "Touches sensitive paths: <list>. Manual review required."
   - Exit 0: do NOT approve
7. Run the merge train gate for shared conflict zones:
...
9. Check CI with `gh pr checks <n> --json name,state,bucket`.
   - The orchestrator normally avoids spawning Reviewer while checks are pending, but verify inside the role anyway.
...
- Don't read complete historical logs. Use the work packet and bounded tails
  unless a specific failure requires a targeted older excerpt.
